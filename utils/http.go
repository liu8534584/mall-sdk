...
func Dingding(url string, msg string) {
	message := `{"msgtype": "text",
		"text": {"content": "` + msg + `"}
	}`
	resp, _ := HttpPostJson(url, message)
	fmt.Println(resp)
}

func WechatAlarmMsg(key string, msg string) {
	message := `{"msgtype": "text",
		"text": {"content": "` + msg + `"}
	}`
	resp, _ := HttpPostJson("https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key="+key, message)
	fmt.Println(resp)
}
