...
type UnionOpenOrderRowQueryResponseTopLevel struct {
	UnionOpenOrderRowQueryResponse UnionOpenOrderRowQueryResponse `json:"jd_union_open_order_row_query_responce"`
}

type UnionOpenOrderRowQueryResponse struct {
...
type OrderRow struct {
	...
	ModifyTime          string  `json:"modifyTime"`          //下单时间
	OrderEmt            int8    `json:"orderEmt"`            //下单设备(1:PC,2:无线)
	Plus                int8    `json:"plus"`                //下单用户是否为PLUS会员 0：否，1：是
	...
	ValidCode           int8    `json:"validCode"`           //订单维度的有效码（-1：未知,2.无效-拆单,3.无效-取消,4.无效-京东帮帮主订单,5.无效-账号异常,6.无效-赠品类目不返佣,7.无效-校园订单,8.无效-企业订单,9.无效-团购订单,10.无效-开增值税专用发票订单,11.无效-乡村推广员下单,12.无效-自己推广自己下单,13.无效-违规订单,14.无效-来源与备案网址不符,15.待付款,16.已付款,17.已完成,18.已结算（5.9号不再支持结算状态回写展示））注：自2018/7/13起，自己推广自己下单已经允许返佣，故12无效码仅针对历史数据有效
	TraceType           int8    `json:"traceType"`           //同跨店：2同店 3跨店
	...
	UnionRole           int8    `json:"unionRole"`           //站长角色：1 推客 2 团长 3内容服务商
